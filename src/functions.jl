...
function open_png(filename::String)
    fp = ccall((:fopen, "libc"), Ptr{Nothing}, (Cstring, Cstring), filename, "rb")
    fp == C_NULL && error("Failed to open $filename")

    header = zeros(UInt8, PNG_BYTES_TO_CHECK)
    header_size = ccall((:fread, "libc"), Csize_t, (Ptr{UInt8}, Cint, Cint, Ptr{Nothing}), header, 1, PNG_BYTES_TO_CHECK, fp)
...
function close_png(fp::Ptr{Nothing})
    ccall((:fclose, "libc"), Cint, (Ptr{Nothing},), fp)
end
